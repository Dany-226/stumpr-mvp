...
export default function FichePatientPage() {
  ...
  const handleExportPDF = () => {
    // Génération PDF côté frontend avec jsPDF
    const doc = new jsPDF();
    ...
    // Couleurs Stumpr
    const primaryColor = [29, 122, 114]; // #1d7a72
    const textColor = [26, 31, 46]; // #1a1f2e
    const mutedColor = [136, 146, 164]; // #8892a4
    ...
    // Helper pour calculer date de renouvellement
    const calculateRenewal = (prescriptionDate, durationYears) => {
      if (!prescriptionDate || !durationYears) return null;
      const prescription = new Date(prescriptionDate);
      const renewal = new Date(prescription);
      renewal.setFullYear(renewal.getFullYear() + durationYears);
    ...
    doc.text("Stumpr — Fiche Patient", pageWidth / 2, y, { align: "center" });
    y += 15;
    ...
    doc.setTextColor(...primaryColor);
    doc.setFont("helvetica", "bold");
    doc.text("IDENTITÉ PATIENT", margin, y);
    y += 10;
    ...
    const identityFields = [
      ...
      ["Date de naissance", formatDate(formData.date_naissance)],
      ["Email", formData.email || "Non renseigné"],
      ["Téléphone", formData.telephone || "Non renseigné"],
      ["Niveau d'activité", formData.niveau_activite || "Non renseigné"]
    ];
    ...
    doc.setTextColor(...primaryColor);
    doc.setFont("helvetica", "bold");
    doc.text("AMPUTATION", margin, y);
    y += 10;
    
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    
    const amputationFields = [
      ["Niveau d'amputation", formData.niveau_amputation || "Non renseigné"],
      ["Côté", formData.cote || "Non renseigné"],
      ["Date d'amputation", formatDate(formData.date_amputation)],
      ["Cause", formData.cause || "Non renseignée"]
    ];
    
    amputationFields.forEach(([label, value]) => {
      doc.setTextColor(...mutedColor);
      doc.text(`${label}:`, margin, y);
      doc.setTextColor(...textColor);
    ...
    if (formData.notes_moignon) {
      y += 4;
      doc.setTextColor(...mutedColor);
      doc.text("Notes sur le moignon:", margin, y);
      y += 6;
      doc.setTextColor(...textColor);
      const moignonLines = doc.splitTextToSize(formData.notes_moignon, pageWidth - 2 * margin);
      doc.text(moignonLines, margin, y);
      y += moignonLines.length * 5;
    }
    y += 10;
    
    // ============ SECTION 3: COMPOSANTS PROTHÉTIQUES ============
    if (formData.composants && formData.composants.length > 0) {
      checkPageBreak(40);
      doc.setFontSize(14);
      doc.setTextColor(...primaryColor);
      doc.setFont("helvetica", "bold");
      doc.text("COMPOSANTS PROTHÉTIQUES (LPPR)", margin, y);
      y += 10;
      
      formData.composants.forEach((comp, index) => {
        checkPageBreak(45);
        ...
        doc.setFont("helvetica", "bold");
        doc.setTextColor(...textColor);
        doc.text(`Composant ${index + 1}`, margin, y);
        y += 7;
        ...
        // Code LPPR
        doc.setTextColor(...mutedColor);
        doc.text("Code LPPR:", margin, y);
        doc.setTextColor(...textColor);
        doc.text(comp.code || "N/A", margin + 30, y);
        y += 5;
        
        // Nomenclature
        doc.setTextColor(...mutedColor);
        doc.text("Nomenclature:", margin, y);
        y += 5;
        doc.setTextColor(...textColor);
        const nomLines = doc.splitTextToSize(comp.nomenclature || "N/A", pageWidth - 2 * margin);
        doc.text(nomLines, margin, y);
        y += nomLines.length * 4 + 2;
        
        // Tarif et Durée sur même ligne
        doc.setTextColor(...mutedColor);
        doc.text("Tarif TTC:", margin, y);
        doc.setTextColor(...textColor);
        doc.text(comp.tarif ? `${comp.tarif}€` : "N/A", margin + 25, y);
        doc.setTextColor(...mutedColor);
        doc.text("Durée prise en charge:", margin + 60, y);
        doc.setTextColor(...textColor);
        doc.text(comp.duree_ans ? `${comp.duree_ans} ans` : "N/A", margin + 105, y);
        y += 5;
        
        // Catégorie et Application
        doc.setTextColor(...mutedColor);
        doc.text("Catégorie:", margin, y);
        doc.setTextColor(...textColor);
        doc.text(comp.categorie || "N/A", margin + 25, y);
        doc.setTextColor(...mutedColor);
        doc.text("Application:", margin + 80, y);
        doc.setTextColor(...textColor);
        doc.text(comp.application || "N/A", margin + 105, y);
        y += 5;
        
        // Date prescription
        doc.setTextColor(...mutedColor);
        doc.text("Date de prescription:", margin, y);
        doc.setTextColor(...textColor);
        doc.text(formatDate(comp.date_prescription), margin + 50, y);
        y += 5;
        
        // Date renouvellement (calculée)
        const renewalDate = calculateRenewal(comp.date_prescription, comp.duree_ans);
        if (renewalDate) {
          doc.setTextColor(...mutedColor);
          doc.text("Date de renouvellement:", margin, y);
          doc.setTextColor(...primaryColor);
          doc.text(renewalDate, margin + 55, y);
          y += 5;
        }
        
        // Prise en charge et état
        if (comp.prise_en_charge_complementaire || comp.etat_composant) {
          doc.setTextColor(...mutedColor);
          doc.text("Prise en charge:", margin, y);
          doc.setTextColor(...textColor);
          doc.text(comp.prise_en_charge_complementaire || "Aucune", margin + 40, y);
          doc.setTextColor(...mutedColor);
          doc.text("État:", margin + 80, y);
          doc.setTextColor(...textColor);
          doc.text(comp.etat_composant || "N/A", margin + 95, y);
          y += 5;
        }
        ...
        if (comp.notes) {
          ...
          y += 4;
          doc.setTextColor(...textColor);
          const notesLines = doc.splitTextToSize(comp.notes, pageWidth - 2 * margin);
    ...
    doc.setTextColor(...primaryColor);
    doc.setFont("helvetica", "bold");
    doc.text("SUIVI MÉDICAL", margin, y);
    y += 10;
    ...
    const medicalFields = [
      ["Orthoprothésiste référent", formData.ortho_referent || "Non renseigné"],
      ["Cabinet / Centre", formData.cabinet_centre || "Non renseigné"],
      ["Téléphone ortho", formData.telephone_ortho || "Non renseigné"],
      ["Médecin prescripteur", formData.medecin_prescripteur || "Non renseigné"],
      ["Spécialité", formData.specialite_prescripteur || "Non renseignée"],
      ["Prochain rendez-vous", formatDate(formData.prochain_rdv)]
    ];
    ...
    if (formData.notes_medicales) {
      y += 4;
      doc.setTextColor(...mutedColor);
      doc.text("Notes médicales:", margin, y);
      y += 6;
      doc.setTextColor(...textColor);
      const medLines = doc.splitTextToSize(formData.notes_medicales, pageWidth - 2 * margin);
    ...
    if (formData.activites && formData.activites.length > 0) {
      ...
      doc.setTextColor(...primaryColor);
      doc.setFont("helvetica", "bold");
      doc.text("ACTIVITÉS QUOTIDIENNES", margin, y);
      y += 10;
      ...
      const activityLabels = formData.activites.map(actId => {
        const act = ACTIVITIES.find(a => a.id === actId);
        return act ? act.label : actId;
      });
      
      const activitiesText = activityLabels.join(" • ");
      const actLines = doc.splitTextToSize(activitiesText, pageWidth - 2 * margin);
      doc.text(actLines, margin, y);
      y += actLines.length * 5 + 10;
    }
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(`Document généré le ${genDate} — Page ${i}/${pageCount}`, pageWidth / 2, 290, { align: "center" });
    }
    
    // Téléchargement automatique
    const fileName = `stumpr-fiche-${(formData.nom || 'patient').toLowerCase().replace(/\s+/g, '-')}.pdf`;
    doc.save(fileName);
